...
public class TicTacToe : MonoBehaviour
{
    ...
    private bool Victory(int result)
    {
        if (arrayCells[0] + arrayCells[1] + arrayCells[2] == result)
            return true;
        if (arrayCells[3] + arrayCells[4] + arrayCells[5] == result)
            return true;
        if (arrayCells[6] + arrayCells[7] + arrayCells[8] == result)
            return true;
        if (arrayCells[0] + arrayCells[3] + arrayCells[6] == result)
            return true;
        if (arrayCells[1] + arrayCells[4] + arrayCells[7] == result)
            return true;
        if (arrayCells[2] + arrayCells[5] + arrayCells[8] == result)
            return true;
        if (arrayCells[0] + arrayCells[4] + arrayCells[8] == result)
            return true;
        if (arrayCells[2] + arrayCells[4] + arrayCells[6] == result)
            return true;
        return false;
    }
